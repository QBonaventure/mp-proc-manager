defmodule MPProcManager.ManiaplanetServer do
  alias __MODULE__
  require Logger
  use GenServer
  ...
  def init(%ServerConfig{} = server_config) do
    ...
    server_config = %ServerConfig{server_config | listening_ports: listening_ports}

    state = %{port: port, controller_port: port, os_pid: os_pid, exit_status: nil, latest_output: nil, status: "running", config: server_config}
    {:ok, _controller_port} =  MPProcManager.Controller.Maniacontrol.start_link([state.config])
